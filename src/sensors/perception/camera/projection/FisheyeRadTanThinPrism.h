...
#include <perception/camera/projection/common.h>
#include <Eigen/Core>
...
class FisheyeRadTanThinPrism {
 public:
  ...
  static ARVR_HOST_DEVICE Eigen::Matrix<typename D::Scalar, 2, 1> project(
  ...
  static ARVR_HOST_DEVICE Eigen::Matrix<typename D::Scalar, 2, 1> unprojectUnitPlane(
  ...
  ARVR_HOST_DEVICE static void scaleParams(typename DP::Scalar s, Eigen::DenseBase<DP>& params) {
    EIGEN_STATIC_ASSERT(
        DP::ColsAtCompileTime == 1 &&
            (DP::RowsAtCompileTime == kNumParams || DP::RowsAtCompileTime == Eigen::Dynamic),
        THIS_METHOD_IS_ONLY_FOR_MATRICES_OF_A_SPECIFIC_SIZE);
  ...
  static ARVR_HOST_DEVICE Eigen::Matrix<typename D::Scalar, 3, 1> unproject(
 ...
 private:
  ...
  ARVR_HOST_DEVICE_INLINE static Eigen::Matrix<T, 2, 1> compute_xr_yr_from_uvDistorted(
  ...
  ARVR_HOST_DEVICE_INLINE static T getThetaFromNorm_xr_yr(
  ...
  template <typename D>
  ARVR_HOST_DEVICE_INLINE static void compute_duvDistorted_dxryr(

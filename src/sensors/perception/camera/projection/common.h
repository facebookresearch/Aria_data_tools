...
#include <perception/math/optimization/IgnoreJetInfinitesimal.h>
#include <portability/Inline.h>
...
template <typename T>
constexpr ARVR_HOST_DEVICE float getConvergenceTolerance() {
  ...
  if (std::is_same<T, apophis2::flat_jets::Jet<float, T::DIMENSION>>::value) {
    return kFloatTolerance;
  }
  if (std::is_same<T, apophis2::flat_jets::Jet<double, T::DIMENSION>>::value) {
    return kDoubleTolerance;
  }
}

template <>
constexpr ARVR_HOST_DEVICE float getConvergenceTolerance<float>() {
  return kFloatTolerance;
}

template <>
constexpr ARVR_HOST_DEVICE float getConvergenceTolerance<double>() {
  return kDoubleTolerance;
}

template <typename T>
inline ARVR_HOST_DEVICE bool hasConverged(const T& step) {
  using std::abs;
  return abs(IgnoreJetInfinitesimal(step)) < getConvergenceTolerance<T>();
}

template <typename T>
inline ARVR_HOST_DEVICE bool hasConverged(const T& stepA, const T& stepB) {
  return IgnoreJetInfinitesimal(stepA * stepA + stepB * stepB) <
      getConvergenceTolerance<T>() * getConvergenceTolerance<T>();
}

template <typename T>
inline ARVR_HOST_DEVICE bool hasConverged(const Eigen::Matrix<T, 2, 1>& step) {
  return IgnoreJetInfinitesimal(step.squaredNorm()) <
      getConvergenceTolerance<T>() * getConvergenceTolerance<T>();
}

template <typename T>
inline ARVR_HOST_DEVICE T initTheta(const T& r) {
  ...
  // use seem to bias towards lower values. This warrents futher inverstigation however with the
  // naive zero distortion initalization some models fail for some points near 90 degrees. The sqrt

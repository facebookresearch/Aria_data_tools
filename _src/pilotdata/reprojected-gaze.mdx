...
In the csv file (x,y) represents the calibrated 2D pixel coordinate of gaze position. Note that the provided timestamp is the timestamp of the eye camera image used to compute the gaze, not an rgb camera timestamp. To query the approximate location of eye gaze at rgb image timestamp, we suggest you interpolate gaze results from neighboring frames.
